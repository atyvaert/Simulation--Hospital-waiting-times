...
class simulation():
    ...
    rule = 3  # integer indicating which scheduling rule you are testing
                ...
                # B) for elective slots: appointment time depends on the RULE
                    ...
                    if (rule == 1):  # FIFO rule
                        ...
                        if s in range(0, K):
                            ...
                            weekSchedule[d][s].appTime = time - Normal_distribution(15, 3, setWeekSchedule_seed,
                                                                                    run2)  # set this appointment time
                            run2 += 1
            ...
            for d in range(0, D):  # not on Sunday
                ...
                if d < D - 1:  # not on Saturday either
                    ...
                    while arrivalTimeNext < 17:  # desk open from 8h until 17h
                        ...
                        tardiness = Normal_distribution(meanTardiness, stdevTardiness, arrivalTime_seed,
                                                        run) / 60.0  # in practice this is not known yet at time  of call
                        noShow = not bool(Bernouilli_distribution(probNoShow, arrivalTime_seed,
                                                                  run))  # in practice this is not known yet at time of call
                        duration = Normal_distribution(meanElectiveDuration, stdevElectiveDuration, arrivalTime_seed,
                                                       run) / 60.0  # in practice this is not known yet at time of call
                        ...
                        counter = counter + 1
                        run += 1
                        arrivalTimeNext = arrivalTimeNext + Exponential_distribution(lambdaElective, arrivalTime_seed,
                                                                                     run) * (
                ...
                while arrivalTimeNext < endTime:  # desk open from 8h until 17h
                    ...
                    duration = Normal_distribution(meanUrgentDuration[scanType], stdevUrgentDuration[scanType],
                                                   arrivalTime_seed,
                                                   run) / 60.0  # in practice this is not known yet at time of arrival
                    patient = Patient(counter, patientType, scanType, w, d, callTime, tardiness, noShow, duration)
                    patients.append(patient)
                    counter = counter + 1
                    run += 1
                    arrivalTimeNext = arrivalTimeNext + Exponential_distribution(lambda_local, arrivalTime_seed,
                                                                                 run) * (
            ...
            if (week[i] < W):
                ...
                if (patient.patientType == 1):
                    if (previousWeek < week[i]):
                        movingAvgElectiveAppWT[previousWeek] = movingAvgElectiveAppWT[
                                                                   previousWeek] / numberOfElectivePerWeek
                        numberOfElectivePerWeek = 0
                        previousWeek = week[i]
    ...
    def sortPatientsOnAppTime(self):
        patients.sort(
            key=lambda x: (x.scanWeek, x.scanDay, x.appTime, x.callWeek, x.callDay, x.callTime, -x.patientType))
        i = 0
        for patient in patients:
    ...
    def runSimulations(self):
        ...
        values_to_add = {'r': r, 'elAppWT': electiveAppWT_SS, 'urScanWT': urgentScanWT_SS,
                         'elScanWT': electiveScanWT_SS,
                         'OT': OT_SS,
